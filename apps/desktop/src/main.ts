...
import { appDisplayName, APP_BASE_NAME } from "@t3tools/shared/branding";
...
const APP_DISPLAY_NAME = appDisplayName(isDevelopment);
/** Unchanged for Windows taskbar / notification continuity (see plan). */
 ...
 * parentheses (e.g. legacy `~/.config/T3 Code (Alpha)` on Linux). This is
  ...
  if (bootstrapStream && "write" in bootstrapStream) {
    bootstrapStream.write(
      `${JSON.stringify({
        ...
        meridianBrainRoot: Path.join(resolveAppRoot(), "meridianBrain"),
