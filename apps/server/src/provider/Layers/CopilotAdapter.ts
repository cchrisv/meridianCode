...
import { buildMeridianTools } from "./meridianTools";
...
import {
  type CopilotSessionKnowledgeConfig,
  buildCopilotSessionKnowledgeConfig,
} from "./copilotSessionKnowledge.ts";
...
interface ActiveCopilotSession extends CopilotTurnTrackingState {
  ...
  /** Last known session token limit from `session.usage_info` (for assistant.usage snapshots). */
  sessionTokenLimit?: number;
  unsubscribe: () => void;
  copilotKnowledgeConfig: CopilotSessionKnowledgeConfig;
}
...
function getCopilotReasoningEffortFromModelSelection(
...
function copilotSessionUsageInfoToSnapshot(data: {
  tokenLimit: number;
  currentTokens: number;
}): ThreadTokenUsageSnapshot {
  return {
    usedTokens: Math.max(0, Math.trunc(data.currentTokens)),
    maxTokens: Math.max(1, Math.trunc(data.tokenLimit)),
  };
}

function copilotAssistantUsageToSnapshot(
  data: {
    model: string;
    inputTokens?: number;
    outputTokens?: number;
  ...
  return {
    usedTokens: Math.max(0, input + output),
    ...(sessionTokenLimit !== undefined && sessionTokenLimit > 0
      ? { maxTokens: Math.max(1, Math.trunc(sessionTokenLimit)) }
      : {}),
    ...(data.inputTokens !== undefined ? { inputTokens: Math.max(0, data.inputTokens) } : {}),
    ...(data.outputTokens !== undefined ? { outputTokens: Math.max(0, data.outputTokens) } : {}),
    ...(data.cacheReadTokens !== undefined
      ? { cachedInputTokens: Math.max(0, data.cacheReadTokens) }
      : {}),
  };
}
...
function createSessionRecord(input: {
  ...
  readonly copilotKnowledgeConfig: CopilotSessionKnowledgeConfig;
  ...
  return {
    ...
    copilotKnowledgeConfig: input.copilotKnowledgeConfig,
  };
}
...
const makeCopilotAdapter = (options?: CopilotAdapterLiveOptions) =>
  Effect.gen(function* () {
          ...
          return [
            {
              ...
              payload: {
                usage: copilotSessionUsageInfoToSnapshot(event.data),
              },
            },
          ];
          ...
          return [
            {
              ...
              type: "thread.token-usage.updated",
              payload: {
                usage: copilotAssistantUsageToSnapshot(event.data, record.sessionTokenLimit),
              },
            },
          ];
        }
    ...
    const reconfigureSession = (
      ...
      Effect.tryPromise({
        try: async () => {
          const serverSettings = await Effect.runPromise(serverSettingsService.getSettings);
          const mcpServers = await loadCopilotMcpServers(
            trimToUndefined(serverSettings.providers.copilot.configDir.trim()) ?? undefined,
          );
          const knowledgeConfig = await buildCopilotSessionKnowledgeConfig(
            serverSettings,
            mcpServers,
          );
          ...
          record.copilotKnowledgeConfig = knowledgeConfig;
    ...
    const handleSessionEvent = (record: ActiveCopilotSession, event: SessionEvent) => {
      ...
      if (event.type === "session.usage_info") {
        record.sessionTokenLimit = Math.max(1, Math.trunc(event.data.tokenLimit));
      }
      if (event.type === "assistant.turn_start") {
      ...
      if (event.type === "session.plan_changed" && event.data.operation !== "delete") {
        void Effect.runPromise(emitLatestProposedPlan(record)).catch((cause: unknown) => {
          void emitRuntimeEvents([
            makeSyntheticEvent(
              record.threadId,
              "runtime.warning",
    ...
    const startSession: CopilotAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const copilotSettings = serverSettings.providers.copilot;
        const cliPath =
          normalizeCopilotCliPathOverride(copilotSettings.binaryPath) ??
          resolveBundledCopilotCliPath();
        const configDir = trimToUndefined(copilotSettings.configDir.trim());
        const selectedModel =
          input.modelSelection?.provider === "copilot" ? input.modelSelection.model : undefined;
        const reasoningEffort = getCopilotReasoningEffortFromModelSelection(input.modelSelection);
        ...
        const knowledgeConfig = yield* Effect.promise(() =>
          buildCopilotSessionKnowledgeConfig(serverSettings, mcpServers),
        );
        ...
        const sessionHandlers = createInteractionHandlers(
          input.threadId,
        ...
        const session = yield* Effect.tryPromise({
          try: async () => {
            const meridianTools = buildMeridianTools();

            if (resumeSessionId) {
              return client.resumeSession(resumeSessionId, {
                onPermissionRequest: sessionHandlers.onPermissionRequest,
                onUserInputRequest: sessionHandlers.onUserInputRequest,
        ...
        const record = createSessionRecord({
          ...
          model: selectedModel,
          reasoningEffort,
          copilotKnowledgeConfig: knowledgeConfig,
        });
    ...
    const sendTurn: CopilotAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const explicitReasoningEffort = getCopilotReasoningEffortFromModelSelection(
          input.modelSelection,
        );
        const selectedFromTurn =
          input.modelSelection?.provider === "copilot" ? input.modelSelection.model : undefined;
        const nextModel = selectedFromTurn ?? record.model;
        const nextReasoningEffort =
          explicitReasoningEffort !== undefined
            ? explicitReasoningEffort
            : selectedFromTurn !== undefined && selectedFromTurn !== record.model
              ? undefined
    ...
    const listSessions: CopilotAdapterShape["listSessions"] = () =>
      Effect.sync(() => {
        ...
        for (const record of sessions.values()) {
          ...
          if (record.cwd !== undefined) o.cwd = record.cwd;
          if (record.model !== undefined) o.model = record.model;
          if (record.currentTurnId !== undefined) o.activeTurnId = record.currentTurnId;
          if (record.lastError !== undefined) o.lastError = record.lastError;
          result.push(o as ProviderSession);
        }
        return result;
      });
    ...
    const compactThread: CopilotAdapterShape["compactThread"] = (_threadId) =>
      Effect.fail(
        new ProviderAdapterValidationError({
          provider: PROVIDER,
          operation: "compactThread",
          issue:
            "Manual context compaction is not available for GitHub Copilot sessions in this client.",
        }),
      );
